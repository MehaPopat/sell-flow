...
import type { Investor, InvestorProfile, UserRole } from "@/types";
...
export default function InvestorProfilePage() {
  ...
  const sessionRole = getSession()?.role ?? "ifa";
  const layoutRole: UserRole = sessionRole === "ops" ? "ops" : "ifa";

  useEffect(() => {
    if (!id) return;
    Promise.all([fetchInvestorById(id), fetchInvestorProfile(id)]).then(([inv, prof]) => {
    ...
    <PortalLayout role={layoutRole}>
      <div className="space-y-6">
        ...
        <div className="text-left">
          ...
          <p className="text-sm text-muted-foreground">Investor profile and KYC details</p>
        </div>
