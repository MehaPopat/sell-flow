...
import { fetchIFAById, fetchInvestorsForIFA } from "@/services/api";
import { PortalLayout } from "@/components/PortalLayout";
import type { Investor } from "@/types";

export default function AdminIFAInvestorListPage() {
  ...
  useEffect(() => {
    if (!ifaId) return;
    Promise.all([fetchInvestorsForIFA(ifaId), fetchIFAById(ifaId)]).then(([invData, ifaData]) => {
      setInvestors(invData);
      if (ifaData) setIfaName(ifaData.name);
      setLoading(false);
    });
  }, [ifaId]);
      ...
      <div className="space-y-6">
        ...
        <div className="text-left">
          <h1 className="text-xl font-semibold">
            {loading ? "Investors" : `${ifaName}'s Investors`}
          </h1>
          <p className="text-sm text-muted-foreground">Investors assigned to this IFA</p>
        </div>
          ...
          <div className="investor-table-wrap">
            <table className="investor-table">
              ...
              <tbody>
                {loading && (
                  <tr>
                    <td colSpan={4} className="investor-empty">Loading investors...</td>
                  </tr>
                )}
                {!loading && filtered.length === 0 && (
                  <tr>
                    <td colSpan={4} className="investor-empty">No investors found</td>
                  </tr>
                )}
                {filtered.map((inv) => (
                  <tr key={inv.id} className="investor-row">
                    <td className="investor-td">
                      <div className="investor-name-cell">
                        <span className="investor-avatar">{inv.name.charAt(0).toUpperCase()}</span>
